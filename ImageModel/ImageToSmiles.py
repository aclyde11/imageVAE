...
train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=325, shuffle=False, drop_last=True, sampler=torch.utils.data.SubsetRandomSampler(list(range(0, data_size))),  **kwargs)
...
def train(epoch):
    with experiment.train():
        ...
        imageGeneratorModel.eval()
        decoder.train()  # train mode (dropout and batchnorm is used)
        encoder.train()
        losses = AverageMeter()  # loss (per word decoded)
        for batch_idx, (embed, data, embedlen) in enumerate(train_loader):
            imgs = data.float().cuda(0)
            recon_im = imageGeneratorModel(imgs).cuda(1)
            ...
            # Forward prop.
            recon_im = encoder(recon_im).cuda(2)

            scores, caps_sorted, decode_lengths, alphas, sort_ind = decoder(recon_im, caps, caplens, teacher_forcing=bool(epoch > 1))
