...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2048 * 2, zero_init_residual=False, in_classes=3):
        ...
        self.inplanes = 16
        self.conv1 = nn.Conv2d(in_channels=in_classes, 16, kernel_size=7, stride=2, padding=3,
                               bias=False)
        ...
        self.layer4 = self._make_layer(block, 56, layers[3], stride=2)
...
class BottleneckTranspose(nn.Module):
    ...
    def __init__(self, planes, inplanes, stride=1, downsample=None):
        super(BottleneckTranspose, self).__init__()
        # Both self.conv2 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv1x1T(inplanes, planes)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = conv3x3T(planes, planes, stride)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = conv1x1T(planes, planes / self.expansion)
        self.bn3 = nn.BatchNorm2d(planes / self.expansion)
        self.relu = nn.ReLU(inplace=True)
