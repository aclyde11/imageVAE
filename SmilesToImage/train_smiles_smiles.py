...
train = torch.utils.data.TensorDataset(data_train, torch.zeros(data_train.shape[0]))
train_loader = torch.utils.data.DataLoader(train, batch_size=3000, shuffle=True)

val = torch.utils.data.TensorDataset(data_val, torch.zeros(data_val.shape[0]))
