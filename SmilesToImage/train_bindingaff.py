...
import torchvision
from model import GeneralVae,  PictureDecoder, PictureEncoder, TestVAE, DenseMolEncoder, ZSpaceTransform, ComboVAE, MolDecoder, AutoModel, BindingAffModel
import pickle
...
def train(epoch, train_loader):
    with experiment.train():
        ...
        model.train()
        train_loss = 0
        for batch_idx, (data, embed, aff) in enumerate(train_loader):
            data = data[0].float().cuda()
            #embed = embed.float().cuda()
            recon_batch, mu, logvar, z = model(data)

            binding_pred = binding(z)

            loss = lossf(recon_batch, data, mu, logvar, epoch)
            loss_binding = binding_loss(aff, binding_pred)

            experiment.log_metric("loss", loss.item())
            experiment.log_metric("binding_loss", loss_binding.item())
...
n_loader = generate_data_loader(train_root, 600, int(100000))
